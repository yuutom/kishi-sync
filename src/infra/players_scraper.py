...
def parse_player_detail(url: str) -> Player:
    try:
        ...
        player_category_symbol = ""
        match = re.search(r"/player/([^/]+)/", url)
        if match:
            player_category_symbol = match.group(1)
        player_category = Enums.PlayerCategory.from_symbol(player_category_symbol)
        ...
        return Player(
            id=f"{player_category.symbol}_{kishi_number}",
